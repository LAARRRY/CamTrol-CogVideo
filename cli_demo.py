...
def concat_warp_start(image, num_frames, concat_path, device = 'cuda'): # 原图在开头
...
def generate_video(
    ...
    if latents != None:
        noisy_latents = torch.load(latents)
        l_latents = latents.split('/')[-1]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path", type=str, default="your_path_to_CogVideoX-5b", help="The path of the pre-trained model to be used"
    )
    ...
    if args.latents != None:
        num_inference_steps = args.latents[args.latents.find('of')+2:args.latents.find('.pt')]
