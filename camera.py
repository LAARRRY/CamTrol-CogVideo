...
def get_pcdGenPoses(pcdgenpath_list, center_depth):
    ...
    if pcdgenpath == 'zoom':
        ...
        if pcdgenpath_list[3] == 'mode_1':
            trajectories = torch.load('assets/trajectory/complex_1.pth').reshape([16, 3, 4])
            render_poses[zero_index:] = trajectories[:frame]
        elif pcdgenpath_list[3] == 'mode_2':
            trajectories = torch.load('assets/trajectory/complex_2.pth').reshape([16, 3, 4])
            render_poses[zero_index:] = trajectories[:frame]
        elif pcdgenpath_list[3] == 'mode_3':
            trajectories = torch.load('assets/trajectory/complex_3.pth').reshape([16, 3, 4])
            render_poses[zero_index:] = trajectories[:frame]
        elif pcdgenpath_list[3] == 'mode_4':
            trajectories = torch.load('assets/trajectory/complex_4.pth').reshape([16, 3, 4])
            render_poses[zero_index:] = trajectories[:frame]
        else:
            trajectories = torch.load(pcdgenpath_list[3]).reshape([14, 3, 4])[:frame]
            render_poses[zero_index:] = trajectories
...
class Warper:
    def __init__(self, H, W):
        ...
        self.rgb_model =  StableDiffusionInpaintPipeline.from_pretrained(
            'your_path_to_stable-diffusion-inpainting', revision='fp16', safety_checker=None, torch_dtype=torch.float16).to('cuda')
        # self.rgb_model =  StableDiffusionInpaintPipeline.from_pretrained(
    ...
    def generate_pcd(self, rgb_cond, prompt, negative_prompt, pcdgen, seed, diff_steps, save_dir=None, save_warps=False, progress=gr.Progress()):
        ...
        for i in iterable_dream:
            ...
            image_curr = self.rgb(
                prompt=prompt, image=image2, # Image.fromarray((image2*255).astype(np.uint8)).save('see.png')
                negative_prompt=negative_prompt[0], generator=generator, num_inference_steps=diff_steps,
                mask_image=mask2, # Image.fromarray((mask2*255).astype(np.uint8)).save('see.png')
            )
            depth_curr = self.d(image_curr)
    ...
    def generate_pcd_wo(self, rgb_cond, prompt, negative_prompt, pcdgen, seed, diff_steps, save_dir=None, save_warps=False, progress=gr.Progress()):
        generator=torch.Generator(device='cuda').manual_seed(-1)
        warped_images = torch.tensor([]).to('cuda')
        image = ToTensor()(rgb_cond)
        image = image * 2.0 - 1.0
